...
Set the Daylighting Method field to SplitFlux. Legacy DElight values are no longer supported and will terminate the simulation with a targeted error message.
...
The \hyperref[daylightingreferencepoint-000]{Daylighting:ReferencePoint} name should be specified that is used for determining the glare. Only one reference point is used to calculate the glare. This input is used with the supported SplitFlux method.
...
Daylight glare from a window depends on occupant view direction. It is highest when you look directly at a window and decreases as you look away from a window. This field specifies the view direction for calculating glare. It is the angle, measured clockwise in the horizontal plane, between the zone y-axis and the occupant view direction. This input is used with the supported SplitFlux method.

\paragraph{Field: Maximum Allowable Discomfort Glare Index}\label{field-maximum-allowable-discomfort-glare-index}

If a daylit zone has windows with shading devices (except exterior screens), the shades will be deployed if the daylight glare at \hyperref[daylightingreferencepoint-000]{Daylighting:ReferencePoint} specified in the Glare Calculation Daylighting Reference Point Name field exceeds the value of this field. To get this type of glare control you have to specify Trigger = Glare, GlareOrSolarOnWindow, GlareOrHorizontalSolar, GlareOrOutsideAirTemp, GlareOrZoneAirTemp or GlareOrZoneLoad in \hyperref[windowpropertyshadingcontrol]{WindowShadingControl} for one or more windows in the zone (see \hyperref[windowpropertyshadingcontrol]{WindowShadingControl}). This input is used with the supported SplitFlux method.
...
This legacy field is retained only for input compatibility. It is ignored for supported SplitFlux daylighting, and any model that still requests DElight will terminate with a targeted error.
...
A third possible source is neglected because it is generally small in double-faÃ§ade cases. This is daylight that is reflected from the surfaces of Z0, passes through an interior window and then reaches a reference point without inter-reflection in the daylit zone.
...
\caption{Vertical section through a double-faÃ§ade building showing daylighting through interior windows. The dashed lines show that (1) reference point RP1 receives direct light from exterior window EW1 via interior window IW1; (2) RP2 receives direct light from EW1 and EW2 via IW2; and (3) RP3 receives direct light from EW2 via IW3. \protect \label{fig:vertical-section-through-a-double-faade}}
\end{figure}
...
\subsection{Legacy DElight Inputs}\label{delight-daylighting-method}

The DElight daylighting engine and the \textbf{Daylighting:DELight:ComplexFenestration} object are no longer supported. EnergyPlus still recognizes these legacy inputs only so it can emit a targeted fatal error during input processing. Migrate such models to \textbf{Daylighting:Controls} with \textbf{Daylighting Method = SplitFlux} and remove any \textbf{Daylighting:DELight:ComplexFenestration} objects.
...
Allow a thermal zone to be daylit & Daylighting:Controls, Daylighting:ReferencePoint & PurchAirWithDaylighting.idf \tabularnewline
...
Control electric lighting response to daylight illuminance level & Daylighting:Controls/Lighting Control Type = Continuous, Stepped or Continuous/Off, plus other fields; or ???. & PurchAirWithDaylighting.idf \tabularnewline
...
Model daylighting through double facade & See â€œDouble Facades: Daylighting through Interior Windowsâ€* & PurchAirWithDoubleFacadeDaylighting.idf \tabularnewline
Add diffusing (translucent) glass & WindowMaterial:Glazing/Solar Diffusing = Yes* & ~ \tabularnewline
Find effect on daylighting of solar reflected from overhangs, neighboring buildings, etc. & Building/SolarDistribution uses â€œwithReflectionsâ€ option & ~ \tabularnewline
